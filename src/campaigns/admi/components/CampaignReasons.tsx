      ...
      {reasons.map((reason, index) => (
        <div key={index} className={`${styles['reason-to-study__block']}`}>
          <div className={`${styles['reason-to-study__icon']}`}>
            <Image src={`https:${getAssetDetails(assets, reason.fields.courseUspImage.sys.id)?.fields.file}`} alt={''} width={115} height={115} />
          </div>
          <h3 className={`${styles['reason-to-study__title']}`}>{reason.fields.courseUspText}</h3>
