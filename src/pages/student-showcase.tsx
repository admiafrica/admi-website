...
import type { FeaturedProject, DisciplineSection, StudentVoice } from '@/types/student-showcase'
...
const STUDENT_VOICES: StudentVoice[] = [
  {
...
export default function StudentShowcasePage() {
  ...
  return (
    <MainLayout footerBgColor="#1a1a1a">
      <PageSEO
        title="Student Showcase | ADMI"
        description="Explore film, animation, design and audio work produced by ADMI students through hybrid learning."
      />

      <div className="w-full">
        {/* ── Hero ── */}
        <section className="bg-gradient-to-br from-[#0A1A18] via-[#050D0C] to-[#0A0A0A] px-4 py-20 xl:px-20">
          <div className="mx-auto flex w-full max-w-screen-xl flex-col items-center gap-10 lg:flex-row lg:justify-between">
            {/* Left */}
            <div className="max-w-[640px]">
              <p className="font-proxima text-[14px] font-bold tracking-[1.2px] text-[#B7D8CF]">/student-showcase</p>
              <h1 className="mt-7 font-fraunces text-[44px] font-bold leading-[1.15] text-white">
              ...
              <p className="mt-7 max-w-[540px] font-proxima text-[16px] leading-[1.7] text-white/80">
                Explore film, animation, design and audio work produced through ADMI&rsquo;s blended model: online theory, live virtual critiques, and on-campus studio production.
              </p>
            ...
            <div className="relative h-[420px] w-full max-w-[520px] overflow-hidden rounded-2xl">
              <div
                ...
                style={{ backgroundImage: "url('https://images.unsplash.com/photo-1630816631475-8ca50f59ba28?auto=format&fit=crop&w=1080&q=80')" }}
              />
              <div className="absolute left-5 top-5 rounded-md bg-[#BA2E36] px-3.5 py-1.5 font-proxima text-[11px] font-bold tracking-[1px] text-white">
                FEATURED PROJECT
              </div>
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/95 via-black/60 to-transparent px-7 pb-7 pt-16">
                <h3 className="font-fraunces text-[24px] font-bold text-white">Shifting Horizons</h3>
                <p className="mt-2 font-proxima text-[13px] text-white/75">Achieng O. &bull; Film Production Diploma &bull; Short Documentary</p>
              </div>
            </div>
          </div>
        </section>
        ...
        {/* ── Discipline Filters ── */}
        <section className="bg-[#F9F9F9] px-4 py-9 xl:px-20">
          <div className="mx-auto w-full max-w-screen-xl">
            <h2 className="font-fraunces text-[30px] font-bold text-[#171717]">Browse by Learning Mode</h2>
        ...
        <section className="bg-white px-4 py-20 xl:px-20">
          <div className="mx-auto w-full max-w-screen-xl">
            <div className="text-center">
              <p className="font-proxima text-[13px] font-bold uppercase tracking-[1.2px] text-[#BA2E36]">HYBRID STUDENT WORK</p>
              <h2 className="mt-2.5 font-fraunces text-[42px] font-bold text-[#171717]">Featured Projects Across Online + Campus Learning</h2>
            ...
            <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-3">
              {FEATURED_PROJECTS.map((proj) => (
                <article key={proj.title} className="group overflow-hidden rounded-xl border border-[#E8E8E8] bg-white">
                  ...
                  />
                  <div className="p-6">
                    <h3 className="font-fraunces text-[20px] font-bold text-[#171717]">{proj.title}</h3>
                    <p className="mt-2 font-proxima text-[14px] text-[#888]">
                      {proj.student} &bull; {proj.programme}
                    </p>
                    <p className="mt-1 font-proxima text-[13px] font-semibold text-[#BA2E36]">{proj.type}</p>
                  </div>
                </article>
              ))}
            </div>
        ...
        {DISCIPLINE_SECTIONS.map((section) => (
          <section key={section.title} style={{ backgroundColor: section.bg }} className="px-4 py-16 xl:px-20">
            <div className="mx-auto w-full max-w-screen-xl">
              <div className="flex items-center justify-between">
                <h2 className="font-fraunces text-[30px] font-bold text-[#171717]">{section.title}</h2>
                <Link href="#" className="font-proxima text-[14px] font-bold text-[#BA2E36]">View All &rarr;</Link>
              </div>
              <div className="mt-8 grid grid-cols-1 gap-5 md:grid-cols-3">
                {section.projects.map((proj) => (
                  <article key={proj.title} className="group overflow-hidden rounded-xl border border-[#E8E8E8] bg-white">
                    ...
                    />
                    <div className="p-5">
                      <h3 className="font-fraunces text-[18px] font-bold text-[#171717]">{proj.title}</h3>
                      <p className="mt-1.5 font-proxima text-[13px] text-[#888]">{proj.student} &bull; {proj.type}</p>
                    </div>
                  </article>
                ))}
              </div>
        ...
        {/* ── Student Voices ── */}
        <section className="bg-[#0F0F0F] px-4 py-20 xl:px-20">
          <div className="mx-auto w-full max-w-screen-xl">
            <div className="text-center">
              <p className="font-proxima text-[13px] font-bold uppercase tracking-[1.2px] text-[#BA2E36]">STUDENT VOICES</p>
              <h2 className="mx-auto mt-2.5 max-w-[900px] font-fraunces text-[38px] font-bold text-white">
            ...
            <div className="mt-10 grid grid-cols-1 gap-5 md:grid-cols-3">
              {STUDENT_VOICES.map((voice) => (
                <article key={voice.name} className="rounded-xl bg-[#1B1B1B] p-7">
                  ...
                  <p className="mt-4 font-proxima text-[14px] font-bold text-[#BDBDBD]">{voice.name} &bull; {voice.discipline}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="bg-gradient-to-br from-[#BA2E36] via-[#8E2028] to-[#1A1A1A] px-4 py-24 text-center text-white xl:px-20">
          <div className="mx-auto w-full max-w-screen-xl">
            <h2 className="font-fraunces text-[44px] font-bold">Build Your Portfolio Through Hybrid Learning</h2>
