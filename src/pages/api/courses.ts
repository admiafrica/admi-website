  ...
  if (req.method === "GET") {
    try {
      ...
      if (!data.items.length) {
        return res
          .status(404)
          .json({ message: `No course found for slug: ${slug}` });
      }
      ...
      const resolveReferences = (fields: Record<string, IContentfulEntry>) => {
        const resolveField = (field) => {
          ...
          if (field?.sys?.type === "Link") {
            ...
            if (linkedType === "Entry") {
              return (
                entries.find((entry) => entry.sys.id === linkedId) || field
              );
            }
          }
          return field;
        ...
        return Object.entries(fields).reduce<Record<string, IContentfulEntry>>(
