    ...
    <section aria-labelledby="courses-faq-heading" className="w-full bg-[#f9f9f9] px-4 py-12 md:px-20 md:py-20">
      ...
      <div className="mx-auto mt-12 max-w-[800px]">
        {faqs.map((faq, index) => {
          ...
          return (
            <div key={`courses-faq-${index}`} className={!isLast ? 'border-b border-[#e0e0e0]' : ''}>
              <button
                type="button"
              ...
              >
                <span className="font-proxima text-base font-semibold text-[#171717]">{faq.question}</span>
                <span className="flex-shrink-0 text-2xl leading-none text-[#999]" aria-hidden="true">
                  {isOpen ? '\u2212' : '+'}
                </span>
              </button>
              ...
              {isOpen && (
                <div
                  ...
                  className="py-2"
                >
                  <p className="font-proxima text-sm leading-relaxed text-[#666]">{faq.answer}</p>
                </div>
              )}
            </div>
          )
