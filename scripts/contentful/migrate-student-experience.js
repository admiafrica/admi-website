/* Migrate Student Experience pages to Contentful (staging by default)
 *
 * Requirements (env):
 ...
 *
 * Run:
 *   node scripts/contentful/migrate-student-experience.js
 */
...
async function ensureSection(environment, section) {
  const fields = {
    ...
    paragraphs: { [locale]: section.paragraphs }
  }

  if (section.bullets && section.bullets.length) {
    fields.bullets = { [locale]: section.bullets }
  }
...
async function main() {
  console.log(`🚀 Migrating Student Experience pages to Contentful (env: ${environmentId})`)
